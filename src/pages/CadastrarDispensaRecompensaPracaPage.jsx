...
const API_CADASTRAR_DISPENSA_RECOMPENSA_PRACA = import.meta.env
  .VITE_API_URL_CADASTRAR_DISPENSA_RECOMPENSA_PRACA;

const API_BUSCAR_PESSOA_POR_MATRICULA = import.meta.env
  .VITE_API_URL_BUSCAR_PESSOA_POR_MATRICULA;

const API_LISTAR_DISPENSA_RECOMPENSA_PRACA_POR_MINUTA = import.meta.env
  .VITE_API_URL_LISTAR_DISPENSA_RECOMPENSA_PRACA_POR_MINUTA;

const API_ATUALIZAR_DISPENSA_RECOMPENSA_PRACA = import.meta.env
  .VITE_API_URL_ATUALIZAR_DISPENSA_RECOMPENSA_PRACA;

const API_EXCLUIR_DISPENSA_RECOMPENSA_PRACA = import.meta.env
  .VITE_API_URL_EXCLUIR_DISPENSA_RECOMPENSA_PRACA;
...
export function CadastrarDispensaRecompensaPracaPage() {
  ...
  async function carregarRegistros() {
    try {
      ...
      const data = await apiFetch(
        `${API_LISTAR_DISPENSA_RECOMPENSA_PRACA_POR_MINUTA}/${idMinuta}`,
        { method: "GET" },
        getValidAccessToken,
      );
  ...
  function handleEditarRegistro(item) {
    ...
    setIdRegistroEdicao(item.id_dispensa_recompensa_praca || null);
  ...
  async function handleExcluirRegistro(item) {
    const idRegistro = item.id_dispensa_recompensa_praca;
    ...
    try {
      ...
      await apiFetch(
        `${API_EXCLUIR_DISPENSA_RECOMPENSA_PRACA}/${idRegistro}`,
        { method: "DELETE" },
        getValidAccessToken,
      );
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (idRegistroEdicao) {
        await apiFetch(
          `${API_ATUALIZAR_DISPENSA_RECOMPENSA_PRACA}/${idRegistroEdicao}`,
          {
            method: "PUT",
        ...
        await apiFetch(
          API_CADASTRAR_DISPENSA_RECOMPENSA_PRACA,
          {
            method: "POST",
            body: JSON.stringify(payload),
          },
  ...
  return (
    <div>
      <PageHeader
        title="Dispensa Recompensa - Praça"
              ...
              <table className="admin-table">
                ...
                <tbody>
                  {registros.map((item) => (
                    <tr
                      key={
                        item.id_dispensa_recompensa_praca ||
