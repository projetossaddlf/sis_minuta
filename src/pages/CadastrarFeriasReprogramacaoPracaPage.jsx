...
const API_CADASTRAR_FERIAS_REPROG_PRACA = import.meta.env
  .VITE_API_URL_CADASTRAR_FERIAS_REPROGRAMACAO_PRACA;

const API_BUSCAR_PESSOA_POR_MATRICULA = import.meta.env
  .VITE_API_URL_BUSCAR_PESSOA_POR_MATRICULA;

const API_LISTAR_FERIAS_REPROG_PRACA_POR_MINUTA = import.meta.env
  .VITE_API_URL_LISTAR_FERIAS_REPROGRAMACAO_PRACA_POR_MINUTA;

const API_ATUALIZAR_FERIAS_REPROG_PRACA = import.meta.env
  .VITE_API_URL_ATUALIZAR_FERIAS_REPROGRAMACAO_PRACA;

const API_EXCLUIR_FERIAS_REPROG_PRACA = import.meta.env
  .VITE_API_URL_EXCLUIR_FERIAS_REPROGRAMACAO_PRACA;
...
export function CadastrarFeriasReprogramacaoPracaPage() {
  ...
  async function carregarRegistros() {
    try {
      ...
      const data = await apiFetch(
        `${API_LISTAR_FERIAS_REPROG_PRACA_POR_MINUTA}/${idMinuta}`,
        { method: "GET" },
        getValidAccessToken,
      );
  ...
  function handleEditarRegistro(item) {
    ...
    setIdRegistroEdicao(item.id_ferias_reprogramacao_praca || null);
  ...
  async function handleExcluirRegistro(item) {
    const idRegistro = item.id_ferias_reprogramacao_praca;
    ...
    try {
      ...
      await apiFetch(
        `${API_EXCLUIR_FERIAS_REPROG_PRACA}/${idRegistro}`,
        { method: "DELETE" },
        getValidAccessToken,
      );
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (idRegistroEdicao) {
        await apiFetch(
          `${API_ATUALIZAR_FERIAS_REPROG_PRACA}/${idRegistroEdicao}`,
          {
            method: "PUT",
        ...
        await apiFetch(
          API_CADASTRAR_FERIAS_REPROG_PRACA,
          {
            method: "POST",
            body: JSON.stringify(payload),
          },
  ...
  return (
    <div>
      <PageHeader
        title="Férias Reprogramação - Praça"
              ...
              <table className="admin-table">
                ...
                <tbody>
                  {registros.map((item) => (
                    <tr
                      key={
                        item.id_ferias_reprogramacao_praca ||
