...
const API_CADASTRAR_ABONO_PRACA = import.meta.env
  .VITE_API_URL_CADASTRAR_ABONO_PRACA;
...
const API_ATUALIZAR_ABONO_PRACA = import.meta.env
  .VITE_API_URL_ATUALIZAR_ABONO_PRACA;

const API_EXCLUIR_ABONO_PRACA = import.meta.env
  .VITE_API_URL_EXCLUIR_ABONO_PRACA;
...
export function CadastrarAbonoPracaPage() {
  ...
  async function carregarRegistros() {
    try {
      ...
      const data = await apiFetch(
        `${API_LISTAR_ABONO_PRACA_POR_MINUTA}/${idMinuta}`,
        { method: "GET" },
        getValidAccessToken,
      );
  ...
  function handleEditarRegistro(item) {
    ...
    setIdRegistroEdicao(item.id_abono_praca || null);
  ...
  async function handleExcluirRegistro(item) {
    const idRegistro = item.id_abono_praca;
    ...
    try {
      ...
      await apiFetch(
        `${API_EXCLUIR_ABONO_PRACA}/${idRegistro}`,
        { method: "DELETE" },
        getValidAccessToken,
      );
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (idRegistroEdicao) {
        await apiFetch(
          `${API_ATUALIZAR_ABONO_PRACA}/${idRegistroEdicao}`,
          {
            method: "PUT",
  ...
  return (
    <div>
      <PageHeader
        title="Abono de Ponto Anual - Praça"
