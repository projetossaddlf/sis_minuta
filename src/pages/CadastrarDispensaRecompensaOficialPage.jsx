...
const API_CADASTRAR_DISPENSA_RECOMPENSA_OFICIAL = import.meta.env
  .VITE_API_URL_CADASTRAR_DISPENSA_RECOMPENSA_OFICIAL;

const API_BUSCAR_PESSOA_POR_MATRICULA = import.meta.env
  .VITE_API_URL_BUSCAR_PESSOA_POR_MATRICULA;

const API_LISTAR_DISPENSA_RECOMPENSA_OFICIAL_POR_MINUTA = import.meta.env
  .VITE_API_URL_LISTAR_DISPENSA_RECOMPENSA_OFICIAL_POR_MINUTA;

const API_ATUALIZAR_DISPENSA_RECOMPENSA_OFICIAL = import.meta.env
  .VITE_API_URL_ATUALIZAR_DISPENSA_RECOMPENSA_OFICIAL;

const API_EXCLUIR_DISPENSA_RECOMPENSA_OFICIAL = import.meta.env
  .VITE_API_URL_EXCLUIR_DISPENSA_RECOMPENSA_OFICIAL;
...
export function CadastrarDispensaRecompensaOficialPage() {
  ...
  async function carregarRegistros() {
    try {
      ...
      const data = await apiFetch(
        `${API_LISTAR_DISPENSA_RECOMPENSA_OFICIAL_POR_MINUTA}/${idMinuta}`,
        { method: "GET" },
        getValidAccessToken,
      );
  ...
  function handleEditarRegistro(item) {
    ...
    setIdRegistroEdicao(item.id_dispensa_recompensa_oficial || null);

    setForm({
      matr: item.matr || "",
      qtd_dias_dispensa: item.qtd_dias_dispensa || "",
  ...
  async function handleExcluirRegistro(item) {
    const idRegistro = item.id_dispensa_recompensa_oficial;
    ...
    try {
      ...
      await apiFetch(
        `${API_EXCLUIR_DISPENSA_RECOMPENSA_OFICIAL}/${idRegistro}`,
        { method: "DELETE" },
        getValidAccessToken,
      );
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const payload = {
        ...
        matr: form.matr,
        qtd_dias_dispensa: Number(form.qtd_dias_dispensa),
      ...
      if (idRegistroEdicao) {
        await apiFetch(
          `${API_ATUALIZAR_DISPENSA_RECOMPENSA_OFICIAL}/${idRegistroEdicao}`,
          {
            method: "PUT",
        ...
        await apiFetch(
          API_CADASTRAR_DISPENSA_RECOMPENSA_OFICIAL,
          {
            method: "POST",
            body: JSON.stringify(payload),
          },
  ...
  return (
    <div>
      <PageHeader
        title="Dispensa Recompensa - Oficial"
        ...
        <div className="content-card">
          ...
          <form onSubmit={handleSubmit} className="form-grid">
            ...
            <div className="form-row-compact">
              <div className="form-group">
                <label htmlFor="qtd_dias_dispensa">Qtd. Dias</label>
                <input
                  id="qtd_dias_dispensa"
                  name="qtd_dias_dispensa"
                  type="number"
                  min="1"
                  value={form.qtd_dias_dispensa}
                  onChange={handleChange}
                  required
                />
              </div>
              ...
              <table className="admin-table">
                ...
                <tbody>
                  {registros.map((item) => (
                    <tr
                      key={
                        item.id_dispensa_recompensa_oficial ||
                        `${item.matr}-${item.dt_inicio_periodo}`
                      }
                    >
                      <td>{item.matr || "-"}</td>
                      <td>{montarNomePessoa(item)}</td>
                      <td>{item.qtd_dias_dispensa || "-"}</td>
